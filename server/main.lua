...
CreateThread(function()
	Wait(1000)
	local dbModule = request('server/db/sql')
	sql = dbModule.Sql
	ShopCache = dbModule.ShopCache
	if pcall(checkox) then canregister = true end
	request('server/framework/main')

	-- CACHE HELPERS: Wrapper functions for smooth transition from GlobalState to ShopCache
	_G.GetShopData = function(shopLabel)
		return ShopCache.GetShopData(shopLabel)
	end

	_G.GetAllShops = function()
		return ShopCache.GetAllShops()
	end

	_G.UpdateShopField = function(shopLabel, field, value)
		return ShopCache.UpdateShopData(shopLabel, field, value)
	end

	_G.UpdateShopFields = function(shopLabel, fields)
		return ShopCache.UpdateShopFields(shopLabel, fields)
	end

	_G.ModifyShopData = function(shopLabel, callback)
		return ShopCache.ModifyShopData(shopLabel, callback)
	end
	...
	-- VALIDATION: Filter out items from shop configs that don't exist in inventory system
	local function ValidateShopItems()
		local invalidItemsFound = {}
		local validatedStores = {}

		-- Create case-insensitive lookup for Items table
		local itemsLower = {}
		for itemName, _ in pairs(Items) do
			itemsLower[itemName:lower()] = true
		end

		for shopName, shopItems in pairs(shared.Storeitems) do
			validatedStores[shopName] = {}
			for i, itemConfig in ipairs(shopItems) do
				local itemName = itemConfig.metadata and itemConfig.metadata.name or itemConfig.name
				local itemNameLower = itemName:lower()

				-- Check if item exists in inventory system (case-insensitive)
				if itemsLower[itemNameLower] or itemNameLower == 'money' then
					-- Item is valid, keep it
					table.insert(validatedStores[shopName], itemConfig)
				else
					-- Item doesn't exist in inventory, skip it
					if not invalidItemsFound[shopName] then
						invalidItemsFound[shopName] = {}
					end
					table.insert(invalidItemsFound[shopName], itemName)
				end
			end
		end
		...
		shared.Storeitems = validatedStores
	end
...
Inventory.SearchItems = function(source, method, item)
	if shared.inventory == 'ox_inventory' then
		...
		-- COMPATIBILITY: Get player object based on framework
		local Player
		...
		-- CRITICAL FIX: Check Player exists BEFORE accessing properties
		if not Player then
			if item == 'money' then
				return 0 -- No player found, return 0 money
			end
			-- Try stash items as fallback
			local items = exports['qb-inventory']:GetStashItems(source)
			if not items then return method == 'count' and 0 or {} end
			-- Process stash items (code continues below)
		end
...
Inventory.GetItems = function()
	if GetResourceState('ox_inventory') ~= 'started' then
		-- COMPATIBILITY: Framework-specific item retrieval
		if shared.framework == 'QBX' then
			return exports.qbx_core:GetItems() or {}
...
CheckItemData = function(data)
	...
	if not found and AllVehicles then -- try vehicles (with safety check)
...
if shared.inventory == 'ox_inventory' then
	exports.ox_inventory:registerHook('buyItem', function(payload)
		...
		if conf then
			...
			-- Verify this is actually an owned shop in our database
			if stores[store] then
				-- Only process if oxShops is enabled OR this is a player-owned shop
				if shared.oxShops or stores[store].owner then
...
-- REMOVED: ModifyFromBankOffline and GetMoneyFromBankOffline
-- These functions are now defined in server/framework/main.lua with SQL injection protection
		...
		if k == data.shop then
			for k,v in pairs(v) do
				if data.index == k and stores[v.label] then
					...
					if stores[v.label].items[itemtype][itemname] == nil then
						stores[v.label].items[itemtype][itemname] = {}
					end

					-- SECURITY: Double-check stock AFTER acquiring lock
...
RemoveStockFromStash = function(data)
	...
	if not data.booth then
		...
		for k,v in pairs(items) do
			...
			stash[v.metadata and v.metadata.name or v.name] = stash[v.metadata and v.metadata.name or v.name] + v.count
		end
	end
					...
					if storeitems then
						for k,v in pairs(storeitems) do
							...
							if item == itemname or not item then
								...
								stores[v2.label].items[itemtype][itemname].stock = stores[v2.label].items[itemtype][itemname].stock + (tonumber(count) or 100)
...
function hasLicense(name, xPlayer)
	...
	if shared.framework == 'ESX' then
		-- ESX uses database-stored licenses
	...
	elseif shared.framework == 'QBCORE' then
		-- QBCORE uses PlayerData metadata
		if xPlayer.PlayerData and xPlayer.PlayerData.metadata and xPlayer.PlayerData.metadata.licences then
			return xPlayer.PlayerData.metadata.licences[name] == true
		end
		return false
	elseif shared.framework == 'QBX' then
		-- QBX uses PlayerData metadata (similar to QBCORE but explicit for future compatibility)
		if xPlayer.PlayerData and xPlayer.PlayerData.metadata and xPlayer.PlayerData.metadata.licences then
			return xPlayer.PlayerData.metadata.licences[name] == true
		end
		return false
...
lib.callback.register('renzu_shops:buyitem', function(source,data)
	...
	if shopConfig then
		isValidShop = true
		-- Validate job/group restrictions if they exist
		if shopConfig.groups then
			local playerJob = xPlayer?.job?.name or xPlayer?.PlayerData?.job?.name
			local hasAccess = false

			for _, allowedJob in pairs(shopConfig.groups) do
		...
		-- Check if it's a valid owned shop type or movable shop
		local storeowned = isStoreOwned(data.shop, data.index)
	...
	-- Check if any items in cart are currently locked
	for k,v in pairs(data.items) do
		...
		-- Reserve locks for all items in cart
		locks[lockKey] = source
		table.insert(lockedItems, lockKey)
	end

	-- Set locks in GlobalState
	GlobalState.ShopLocks = locks

	-- Helper function to release all locks and clean up purchase order
	local function releaseLocks()
		local currentLocks = GlobalState.ShopLocks or {}
		for _, lockKey in ipairs(lockedItems) do
			currentLocks[lockKey] = nil
		end
		GlobalState.ShopLocks = currentLocks
		-- Clean up purchase order tracking
		purchaseorders[source] = nil
	end
	...
	-- SECURITY: Validate currency type exists before processing
	...
	if money >= total and total > 0 then
				...
				for k2,v in pairs(data.items) do
					...
					if item.name == name then
						...
						if v.count > item.count then
							data.items[k2].count = data.items[k2].count - item.count
						else
							data.items[k2] = nil
						end
						break
		...
		for k,v in pairs(data.items) do
			...
			if data.shop ~= 'VehicleShop' then -- add new item if its not a vehicle type
				...
				for i = 1, tonumber(v.count) do
					...
					-- COMPATIBILITY: Database compatibility layer for vehicle insertion
					local sqldata
					if shared.framework == 'ESX' then
						-- ESX schema: plate, vehicle, owner, stored, job
						sqldata = {
							plate,
					...
					elseif shared.framework == 'QBCORE' or shared.framework == 'QBX' then
						-- QBCORE/QBX schema: plate, mods, license, state, job, citizenid, hash, garage, vehicle
						sqldata = {
							plate,
							json.encode({model = GetHashKey(v.data.name), plate = plate, modLivery = tonumber(v.vehicle?.livery or -1), color1 = tonumber(v.vehicle?.color or 0)}),
							xPlayer.identifier,
		...
		-- AUDIT: Log successful purchase for tracking and debugging
		local purchaseLog = {
			timestamp = os.time(),
			...
			financed = data.finance ~= nil
		}

		-- Build item list for log
		for k,v in pairs(data.items) do
			table.insert(purchaseLog.items, {
				name = v.data.name,
				count = v.count,
				price = data.data[v.data.name] and data.data[v.data.name].price or 0
			})
		end
...
RegisterFinance = function(data)
	...
	if finance[data.identifier].financed then
		...
		finance[data.identifier].max = finance[data.identifier].max - data.total
				...
				for k,v in pairs(v.financed) do
					...
					if xPlayer then
						if xPlayer.getAccount('bank').money >= v.daily then
							...
							finance[debtor].max = finance[debtor].max + v.daily
							finance[debtor].financed[k].total = finance[debtor].financed[k].total - v.daily
							finance[debtor].financed[k].days = finance[debtor].financed[k].days - 1
							if finance[debtor].financed[k].total <= 0 or finance[debtor].financed[k].days == 0 then
								finance[debtor].financed[k] = nil
							end
						else -- penalty fee
							finance[debtor].max = finance[debtor].max - finance[debtor].max/10
							finance[debtor].financed[k].total = finance[debtor].financed[k].total + v.daily/10
						end
		...
		if data then
			for k,v in pairs(data) do
				if metadata and result[v.metadata.name] and not slot[v.slot] then
					result[v.metadata.name] = result[v.metadata.name] + v.count
					slot[v.slot] = true
...
AddMovableShopToPlayer = function(data,source)
	...
	if data.shop.type == 'vehicle' then
		...
		-- COMPATIBILITY: Database compatibility layer for movable vehicle shop insertion
		local sqldata
		if shared.framework == 'ESX' then
			-- ESX schema: plate, vehicle, owner, stored, job
			sqldata = {
				plate,
				json.encode({model = data.shop.model, plate = plate, modLivery = -1}),
				data.owner,
				1,
				'civ'
			}
...
lib.callback.register('renzu_shops:editstore', function(source,data)
	...
	elseif owned and stores[data.store] and data.type == 'deposit_item' then
		-- SECURITY: Runtime item validation
		local itemName = data.metadata and data.metadata.name or data.item
		local inventoryItems = Inventory.GetItems()
		local itemExists = false

		-- Case-insensitive check
		for invItemName, _ in pairs(inventoryItems) do
			if invItemName:lower() == itemName:lower() or itemName:lower() == 'money' then
				itemExists = true
				break
			end
		end
