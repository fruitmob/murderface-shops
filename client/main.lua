self = self or {}  -- Don't wipe out functions from init.lua
self.shopopen = false
self.Items = {}
...
self.addTarget = function(coord, msg, callback, server, var, delete, auto)
	...
	-- REFACTORED: Use ox_target instead of qb-target (Qbox compatible)
		...
		options = {
			{
				...
				label = msg,
				groups = var.shop and var.shop.groups or nil, -- ox_target format
				distance = 2.5,
...
self.Add = function(coord,msg,callback,server,var,delete,auto)
	...
	local sphere = lib.zones.sphere({ index = GetRandomIntInRange(1,9999) ,var = lib.table.deepclone(var) , coords = coord, radius = 2.5, debug = false, inside = inside, onEnter = onEnter, onExit = onExit })
	...
	for name,shops in pairs(shared.OwnedShops) do
		for k,shop in pairs(shops) do
			...
			elseif storedata and storedata.owner == self.PlayerData.identifier
				or storedata and storedata.employee[self.PlayerData.identifier] or shop.groups and self.PlayerData and self.PlayerData.job and self.PlayerData.job.name and self.PlayerData.job.name == self.GetJobFromData(shop.groups) then
				if not shared.target then
...
self.CreateBubbleSpeech = function(data)
	...
	CreateThread(function()
		while self.bubblems > 0 do
			Wait(100)
			self.bubblems = self.bubblems - 100
		end
		return
	end)
	while self.bubblems > 0 do
